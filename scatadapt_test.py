...
def nextItem (itemBank, model = None, theta = 0, out = [], x = [], 
    ...
    if crit == "MFI":
        ...
        if len(keep)==1:
            ...
            select=np.random.choice(keep, 1)[0]
...
def checkStopRule(th, se, N, stop, it = None, model = None, D=1):
    res = False
    res_rule = None
    for i in range(len(stop['rule'])):
        ...
        if ind == 4:
            ...
            if max(info) <= stop['thr'][i]:
                res = True
                res_rule =  stop['rule'][i]
    return {'decision': res, 'rule': res_rule}
...
def thetaEst(it, x, model = None, D = 1, method = "BM", priorDist = "norm", priorPar = np.array([0, 1]), weight = "Huber", 
    ...
    if constantPattern(x) or model != None or constantPatt == None:  
        ...
        if constantPatt in ["BM", "EAP", "WL"]: 
            ...
            if sum(x) == 0:
                dict_for_pat={ 'fixed4' : current_th - 0.4, 'fixed7' : current_th - 0.7,
                                             'var' : 0.5 * (current_th + bRange[0])}
                if constantPatt in dict_for_pat:
                    res=dict_for_pat[constantPatt]
                else:
                    res=0
            else: 
                dict_for_pat={ 'fixed4' : current_th + 0.4, 'fixed7' : current_th + 0.7, 
                                              'var' : 0.5 * (current_th + bRange[1])}
                if constantPatt in dict_for_pat:
                    res=dict_for_pat[constantPatt]
                else:
                    res=0 
        ...
        if sem_exact and model == None:
            ...
            if method == "EAP":
                ...
                if model == None:
                    ...
                    RES = 1/np.sqrt(optI)
    return(RES)    
data=np.genfromtxt('albets2.csv',delimiter=';',skip_header=1,dtype=None,usecols=[1,2])
data = np.hstack((data, np.atleast_2d(np.array([0 for i in range(data.shape[0])])).T,np.atleast_2d(np.array([1 for i in range(data.shape[0])])).T))
m=[]
x=[]
bank=np.array([i for i in range(30)])
ans=[i for i in range(30)]
k=startItems(data, seed=4, nrItems = 2)['items']
for var in bank[k]:
    print(var)
    m.append(var)
    l=int(input())
    if l==ans[var]:
        ...
        x.append(0)
p=data[k]
eap=thetaEst(p,np.array(x),method='EAP',priorPar=[0,2],parInt=[-2,2,33],constantPatt='fixed4',rang=[-2,2],bRange = [min(data[:,1]),max(data[:,1])])
bm=thetaEst(p,np.array(x),current_th=eap,method='BM',priorPar=[0,2],parInt=[-2,2,33], constantPatt='fixed4',rang=[-2,2],bRange = [min(data[:,1]),max(data[:,1])])
while True:
    bmse=semTheta(bm,p,x=np.array(x),method='BM',priorPar=[0,2],constantPatt='fixed4',rang=[-2,2])
    print(bm)
    print(bmse)
    stop = {'rule':["length", "precision"], 
               'thr':[20, 0.39]}
    if checkStopRule(th = bm, it=p,se = bmse, N=len(m), stop = stop)['decision']:
        break
    j=nextItem(data,x=x,theta=bm, out=m,method='BM',priorPar=[0,2],rang=[-2,2])['item']
    print(bank[j][0])
    m.append(j[0][0])
    l=int(input())
    if l==ans[bank[j][0]]:
        ...
        x.append(0)
    p=data[m]
    bm=thetaEst(p,np.array(x),current_th=bm,method='BM',priorPar=[0,2],parInt=[-2,2,33], constantPatt='fixed4',rang=[-2,2],bRange = [min(data[:,1]),max(data[:,1])])
print(bm+3)
